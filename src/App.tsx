              ...
              <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <span className="absolute left-3 top-3 text-slate-400 z-10 animate-pulse-subtle">
                        <Calendar className="w-3.5 h-3.5" />
                      </span>
                      <button
                        type="button"
                        onClick={() => setIsDurationDropdownOpen(!isDurationDropdownOpen)}
                        className="w-full pl-9 pr-6 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold hover:bg-slate-100/50 transition-all text-slate-800 text-left flex items-center justify-between cursor-pointer focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 shadow-3xs"
                      >
                        <span className="truncate">{selectedDuration}</span>
                        <svg 
                          className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-300 shrink-0 ${isDurationDropdownOpen ? 'rotate-180' : ''}`} 
                          fill="none" 
                          stroke="currentColor" 
                          strokeWidth="2.5" 
                          viewBox="0 0 24 24"
                        ...
                        {isDurationDropdownOpen && (
                          <>
                            ...
                            <motion.div
                              ...
                              transition={{ type: 'spring', damping: 25, stiffness: 350 }}
                              className="absolute left-0 right-0 mt-1.5 bg-white border border-slate-200/90 rounded-xl shadow-lg py-1 z-40 overflow-hidden origin-top"
                            >
                              {['2 Days, 1 Night', '3 Days, 2 Nights', '5 Days, 4 Nights'].map((val) => (
                                <button
                                  type="button"
                                  key={val}
                                  onClick={() => {
